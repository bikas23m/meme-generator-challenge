...
function showMeme() {
  const randomMemeLink = getRandomData('memes')
  const memeContainerElement = document.querySelector('.meme-container');
  const memeElement = document.createElement('img');
  memeElement.setAttribute('src', randomMemeLink);
  clearAll()
  memeContainerElement.appendChild(memeElement);
}
...
function showJoke() {
  ...
  memeElement.innerHTML = randomJokeText;
  clearAll();
  jokeContainerElement.appendChild(memeElement);
...
function showRiddle() {
  ...
  const riddleContainerElement = document.querySelector('.riddle-container');

  const riddleElementQustion = document.createElement('p');
  riddleElementQustion.setAttribute('class','riddle-qst');
  riddleElementQustion.innerHTML = randomRiddle.question;

  const riddleElementAnswer = document.createElement('p');
  riddleElementAnswer.setAttribute('class','riddle-ans');
  riddleElementAnswer.innerHTML = 'Ans:'+randomRiddle.answer;
  riddleElementAnswer.hidden = true;

  clearAll();

  
  riddleContainerElement.appendChild(riddleElementQustion);
  riddleContainerElement.appendChild(riddleElementAnswer);

}

function revealAnswers() {
  const riddleElementAnswer = document.querySelector('.riddle-ans');
  if(riddleElementAnswer === null)  alert('there is no riddle');
  if(!riddleElementAnswer.hidden) alert('answer is already revealed')
  else  riddleElementAnswer.hidden=false;
}
